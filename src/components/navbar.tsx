    ...
    return <div className="nav">
      ...
      <div className="navbar  fixed w-full top-0 z-[50] shadow-md">
        ...
        <div className="navbar-title flex gap-4">
          <h1 onClick={()=>navigate('/')} className='logo-link'>Maghh Store</h1>
        </div>
        <div className="navbar-menu md:flex gap-4 hidden">
          <NavLink to={"/deals"} className='nav-link flex items-center gap-1'><Flame/> Deals</NavLink>
          <NavigationMenu>
  <NavigationMenuList>
    <NavigationMenuItem>
      ...
      <NavigationMenuContent className='p-4'>
        <ul className="grid grid-cols-2 space-x-8 divide-x-2 xl:w-[600px] lg:w-[550px] md:w-[400px]">
          <li>
          <NavigationMenuLink className='font-semibold select-none'>Categories</NavigationMenuLink>
            <ul className='w-[300px] lg:h-[250px] flex flex-col gap-1 flex-wrap pt-2'>
              {categories.map(category=>{
                return <li key={category.categoryId}>
                  <NavigationMenuLink className='nav-link'>{category.categoryTitle}</NavigationMenuLink>
                  <div className="flex flex-col gap-1">
                    {category.subCategories?.map(subcat=>{
                      return <NavigationMenuLink key={subcat.categoryId} className='nav-link ms-1'>{subcat.categoryTitle}</NavigationMenuLink>
                    })}
                  </div>
                </li>
              })}
          ...
          <li className='ps-5'>
            <NavigationMenuLink className='font-semibold select-none'>Brands</NavigationMenuLink>
              <ul className='w-[300px] lg:grid grid-cols-2 flex flex-col gap-1 pt-2'>
                {brands.map(brand=>{
                  return <li key={brand.brandId}>
                    <NavigationMenuLink className='nav-link'>{brand.brandTitle}</NavigationMenuLink>
                  </li>
                })}
              </ul>
            </li>
        ...
        <div className="navbar-account flex items-center gap-4">
            ...
            <Badge count={shoppingCartItems.length} style={{ backgroundColor: '#272E3F' }}>
              <ShoppingCart onClick={()=>navigate('/shopping-cart')} className='cursor-pointer hover:text-slate-700'/>
            </Badge>
